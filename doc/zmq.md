...
The Xeke Core daemon can be configured to act as a trusted "border
router", implementing the xeke wire protocol and relay, making
...
The ZeroMQ feature in Xeke Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xeked:
...
For instance:
    ...
    $ xeked -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/xeked.tx.raw \
...
These options can also be provided in xeke.conf.
...
From the perspective of xeked, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xeked directly. Furthermore, no information is
...
using. Xeked appends an up-counting sequence number to each
