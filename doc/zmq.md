...
The Akax Core daemon can be configured to act as a trusted "border
router", implementing the akax wire protocol and relay, making
...
The ZeroMQ feature in Akax Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building akaxd:
...
For instance:
    ...
    $ akaxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/akaxd.tx.raw \
...
These options can also be provided in akax.conf.
...
From the perspective of akaxd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into akaxd directly. Furthermore, no information is
...
using. Akaxd appends an up-counting sequence number to each
