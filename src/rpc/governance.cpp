...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 AKAX irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on akaxcentral etc. and do not save them...
...
static void gobject_vote_conf_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"gobject vote-conf",
        "Vote on a governance object by masternode configured in akax.conf\n",
        {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("akax.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by masternode configured in akax.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Akax features */
    { "akax",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "akax",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "akax",               "gobject",                &gobject,                {} },
    { "akax",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
