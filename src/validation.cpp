...
#if defined(NDEBUG)
# error "Xeke Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Xeke
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if(nPrevHeight <= 600009)
    {
       nSubsidyBase = 5000;
    }
    else if(nPrevHeight > 600009 && nPrevHeight < 1790990 )
    {
	nSubsidyBase = 3500;
    }
    else if(nPrevHeight > 1790990 && nPrevHeight < 6459076)
    {
	nSubsidyBase = 2500;
    }
    else if(nPrevHeight > 6459076 && nPrevHeight < 24906543)
    {
	nSubsidyBase = 70;
    }
    else if(nPrevHeight > 24906543)
    {
	nSubsidyBase = 10;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, int nReallocActivationHeight)
{
   ...
   if (nHeight >= 40000){
    CAmount ret = blockValue/1.6;
    return ret;
   } else {
    CAmount ret = blockValue/60;
    return ret;
   }
}
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    if (consensusparams.BIP66Height) {
        flags |= SCRIPT_VERIFY_DERSIG;
    }

    // Start enforcing CHECKLOCKTIMEVERIFY (BIP65) rule
    if (consensusparams.BIP65Height) {
        flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
    }

    // Start enforcing BIP68 (sequence locks) and BIP112 (CHECKSEQUENCEVERIFY) using versionbits logic.
    if (consensusparams.BIPCSVHeight) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (chainparams.GetConsensus().BIPCSVHeight) {
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(XEKE): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // XEKE : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = llmq::quorumInstantSendManager->GetConflictingLock(*tx)) {
                if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(XEKE): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.DoS(10, error("ConnectBlock(XEKE): transaction %s conflicts with transaction lock %s",
    ...
    // XEKE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(XEKE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.DoS(0, error("ConnectBlock(XEKE): couldn't find masternode or superblock payments"),
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(XEKE): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
