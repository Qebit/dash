This directory contains integration tests that test xeked and its
...
xeked and dash-qt by interacting with them through the RPC and P2P
interfaces.
...
The P2P and RPC ports used by the xeked nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another xeked
process running on the system (perhaps from a previous test which hasn't successfully
killed all its xeked nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other xeked processes are running.

On linux, the test_framework will warn if there is another
xeked process running when the tests are started.

If there are zombie xeked processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
xeked processes running on the system, so should not be used if any non-test
xeked processes are being run.**
...
sure xeked processes are stopped as above):
...
- when run on Travis, no logs are output to the console. However, if a test
  fails, the `test_framework.log` and xeked `debug.log`s will all be dumped
  to the console to help troubleshooting.

To change the level of logs output to the console, use the `-l` command line
argument.

`test_framework.log` and xeked `debug.log`s can be combined into a single
...
call methods that interact with the xeked nodes-under-test.

If further introspection of the xeked instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/xeked.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/xeked <pid>
```
