...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave akaxds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop akaxds after the test execution")
        ...
        parser.add_argument("--akaxd-arg", dest="akaxd_extra_args", default=[], action="append",
                            help="Pass extra args to all akaxd instances")
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/akaxd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/dash-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.akaxd_extra_args
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: akaxds were not stopped and may still be running")
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as akaxd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run akaxds:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in akax.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if akaxd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("akaxd has not been built with zmq enabled.")
