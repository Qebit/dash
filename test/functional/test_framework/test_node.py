...
"""Class for xeked node under test"""
...
class TestNode():
    """A class for representing a xeked node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any xeked processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time xeked is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by xeked, and
        ...
        self.running = True
        self.log.debug("xeked started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the xeked process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'xeked exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. xeked still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to xeked")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to xeked
        expected_msg: regex that stderr should match when xeked fails

        Will throw if xeked starts without an error.
        Will throw if an expected_msg is provided and it does not match xeked's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('xeked failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "xeked should have exited with an error"
                else:
                    assert_msg = "xeked should have exited with expected error " + expected_msg
