...
"""Class for akaxd node under test"""
...
class TestNode():
    """A class for representing a akaxd node under test.
    ...
    def __init__(self, i, datadir, extra_args_from_options, *, chain, rpchost, timewait, bitcoind, bitcoin_cli, mocktime, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False):
        ...
        self.chain = chain
        self.bitcoinconf = os.path.join(self.datadir, "akax.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any akaxd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time akaxd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by akaxd, and
        ...
        self.running = True
        self.log.debug("akaxd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the akaxd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'akaxd exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. akaxd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to akaxd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to akaxd
        expected_msg: regex that stderr should match when akaxd fails

        Will throw if akaxd starts without an error.
        Will throw if an expected_msg is provided and it does not match akaxd's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('akaxd failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "akaxd should have exited with an error"
                else:
                    assert_msg = "akaxd should have exited with expected error " + expected_msg
